    ...
    public class Comment : BaseTable, IBody, IOrgSpecific
    {
        ...
        private async Task ParseMentionsAsync(IDbConnection connection, Comment comment, UserProfile userProfile)
        {
            ...
            if (workItem.DeveloperUserId.HasValue && workItem.DeveloperUserId != sender.UserId)
            {
                ...
                if (!mentionedUsers.Contains(devUser) && devUser.AllowNotification())
                {
                    await AddMentionEventInnerAsync(connection, comment, senderName, devUser);
                    mentionedUsers.Add(devUser);
                }
            }
            ...
            if (workItem.BusinessUserId.HasValue && workItem.BusinessUserId != sender.UserId)
            {
                ...
                if (!mentionedUsers.Contains(bizUser) && bizUser.AllowNotification())
                {
                    await AddMentionEventInnerAsync(connection, comment, senderName, bizUser);
                    mentionedUsers.Add(bizUser);
                }
            }

            int authorUserId = await UserProfile.GetUserIdAsync(connection, workItem.CreatedBy);
            if (authorUserId != sender.UserId)
            {
                var authorUser = await connection.FindWhereAsync<OrganizationUser>(new { workItem.OrganizationId, UserId = authorUserId });
                if (!mentionedUsers.Contains(authorUser) && authorUser.AllowNotification()) await AddMentionEventInnerAsync(connection, comment, senderName, authorUser);
            }            
        }
