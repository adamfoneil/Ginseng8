...
namespace Ginseng.Mvc.Pages.Data
{
	...
	public class IndexModel : AppPageModel
	{
		...
		public IEnumerable<Project> ReferencingProjects { get; set; }
		...
		{			
			using (var cn = Data.GetConnection())
			{
				...
				ReferencingProjects = (CurrentModel != null) ?
					await new Projects() { AppId = CurrentModel.ApplicationId, DataModelId = ModelId, IsActive = true }.ExecuteAsync(cn) :
		...
		public async Task<IActionResult> OnPostDeletePropertyAsync(int id)
		{
			...
			await Data.TryDeleteAsync<ModelProperty>(mp.Id);
		...
		public async Task<IActionResult> OnPostDeleteClassAsync(int id)
		{
			...
			await Data.TryDeleteAsync<ModelClass>(mc.Id);
